...
function buildGraph() {
  ...
  nodes.value = sortedTasks.flatMap((task) => {
    ...
    if (task.isGroup()) {
      const childNodes= task.children().map(child => {return toNode(child,task.id())})
  ...
  // console.log(JSON.stringify(edges.value, null, 2))
...
async function layoutGraph() {
  nodes.value = await layoutWithELK(nodes.value, edges.value, {
    direction: 'RIGHT',
    spacing: 60,
    parentPadding: 40, // 父节点包围子节点的内边距
    topPaddingForLabel:80
  });
...
<template>
  ...
  <VueFlow v-model:nodes="nodes" v-model:edges="edges" class="flow-container" @nodes-initialized="layoutGraph('LR')"   fit-view-on-init elevate-edges-on-select  >
